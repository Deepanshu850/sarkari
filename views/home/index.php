...
<section class="relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-16 relative">
        <div class="grid lg:grid-cols-2 gap-6 lg:gap-12 items-start">
            <!-- LEFT: Conversion Copy -->
            <div class="lg:pt-4">
                <!-- Live counter -->
                <div class="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-3 py-1 text-xs sm:text-sm mb-3 sm:mb-5 backdrop-blur">
                    <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                    <span class="text-green-300 font-medium"><span id="liveCount">17</span> log abhi ye page dekh rahe hain</span>
                </div>

                <h1 class="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black leading-[1.15] text-white mb-3 sm:mb-5">
                    Exam Pass Karna Hai?<br>
                    <span class="text-saffron-400">Pehle Plan Banao.</span>
                </h1>

                <p class="text-sm sm:text-lg text-gray-300 mb-4 sm:mb-6 leading-relaxed max-w-xl">
                ...
                <div class="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6">
                    ...
                    <div class="flex items-start gap-2">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-india-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                        <span class="text-gray-300 text-xs sm:text-sm"><?= $b ?></span>
                    </div>
                    <?php endforeach; ?>
                </div>

                <!-- Mobile: Quick social proof strip instead of full testimonials -->
                <div class="flex items-center gap-2 mb-4 lg:hidden">
                    <div class="flex -space-x-2">
                        <?php for ($i = 0; $i < 4; $i++): ?>
                        <div class="w-6 h-6 rounded-full bg-gradient-to-br from-saffron-400 to-india-500 border-2 border-white/20 flex items-center justify-center text-white text-[8px] font-bold"><?= ['P','R','A','V'][$i] ?></div>
                        <?php endfor; ?>
                    </div>
                    <span class="text-xs text-gray-400"><strong class="text-green-400">2,847+</strong> aspirants ne blueprint liya</span>
                    <span class="text-yellow-400 text-xs">★★★★★</span>
                </div>

                <!-- Desktop: Full testimonials (hidden on mobile - shown in separate section below) -->
            ...
            <div id="get-blueprint" class="lg:sticky lg:top-24">
                <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div class="bg-saffron-500 p-3 sm:p-4 text-center relative overflow-hidden">
                    ...
                    <form method="POST" action="/checkout" class="p-4 sm:p-5 md:p-6 space-y-3 sm:space-y-4">
                        <?= csrf_field() ?>
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:gap-3">
                            <div>
                                <label class="block text-xs sm:text-sm font-semibold text-navy-600 mb-1">Naam *</label>
                                <input type="text" name="name" required placeholder="Poora naam" value="<?= old('name') ?>"
                                    class="w-full px-3 py-2.5 sm:px-4 sm:py-3 bg-cream border border-gold-200 rounded-lg focus:ring-2 focus:ring-saffron-500/30 focus:border-saffron-500 text-navy-600 text-sm">
                            </div>
                            <div>
                                <label class="block text-xs sm:text-sm font-semibold text-navy-600 mb-1">WhatsApp *</label>
                        ...
                        <div>
                            <label class="block text-xs sm:text-sm font-semibold text-navy-600 mb-1">Email *</label>
                        ...
                        <div>
                            ...
                            <div class="flex gap-1 sm:gap-1.5 overflow-x-auto pb-1.5 scrollbar-hide exam-tabs" role="tablist">
                                ...
                                <button type="button"
                                    ...
                                    class="exam-tab-btn flex-shrink-0 px-2 py-1 sm:px-3 sm:py-1.5 rounded-md sm:rounded-lg text-[10px] sm:text-xs font-bold border transition whitespace-nowrap <?= $tabIdx === 0 ? 'bg-saffron-500 text-white border-saffron-500' : 'bg-white text-navy-600 border-gold-200 hover:border-saffron-400' ?>">
                                    <?= e($cat) ?>
                                </button>
                                <?php endforeach; ?>
                            </div>

                            <!-- Exam Buttons per Tab - smaller on mobile -->
                            <?php foreach ($examGroups as $cat => $exams):
                                $tabSlug = preg_replace('/[^a-z0-9]/i', '_', strtolower($cat));
                                $isFirst = ($cat === array_key_first($examGroups));
                            ?>
                            <div id="tab-<?= $tabSlug ?>" class="exam-tab-panel mt-1.5 flex flex-wrap gap-1 sm:gap-1.5 <?= $isFirst ? '' : 'hidden' ?>">
                                <?php foreach ($exams as $exam): ?>
                                <button type="button"
                                    data-exam-id="<?= $exam['id'] ?>"
                                    data-exam-name="<?= e($exam['name']) ?>"
                                    onclick="selectExam(this)"
                                    class="exam-btn px-2 py-1 sm:px-2.5 sm:py-1.5 bg-white border border-gold-200 rounded-md sm:rounded-lg text-[10px] sm:text-xs font-semibold text-navy-600 hover:border-saffron-400 hover:bg-saffron-50 transition">
                                    <?= e($exam['name']) ?>
                                </button>
                                <?php endforeach; ?>
                            </div>
                            ...
                            <p id="examPickerError" class="text-red-500 text-[10px] sm:text-xs mt-1 hidden">Kripya ek exam select karein</p>
                            <p id="examPickerSelected" class="text-india-600 text-[10px] sm:text-xs font-semibold mt-1 hidden"></p>
                        </div>
                        ...
                        <div class="flex items-center justify-center gap-2 sm:gap-3 text-[10px] sm:text-[11px] text-gray-400 pt-0.5 sm:pt-1">
                            <span class="flex items-center gap-1">
                                <svg class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-india-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"/></svg>
                                Razorpay Secure
                            </span>
...
<!-- ========== MOBILE TESTIMONIALS (below hero, only on mobile) ========== -->
<section class="py-6 bg-parchment lg:hidden">
    <div class="max-w-md mx-auto px-4">
        <p class="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-3 text-center">Real Results:</p>
        <div class="flex gap-2.5 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-hide">
            ...
            ?>
            <div class="flex-shrink-0 w-60 snap-start bg-white rounded-lg p-3 border border-gold-200 shadow-sm">
                <div class="flex items-center gap-1 mb-1.5">
                    <?php for ($s = 0; $s < $r['stars']; $s++): ?><span class="text-yellow-400 text-xs">★</span><?php endfor; ?>
                </div>
                <p class="text-gray-700 text-xs leading-relaxed">"<?= $r['text'] ?>"</p>
                <p class="text-[10px] text-gray-400 mt-1.5">— <strong class="text-navy-600"><?= $r['name'] ?></strong>, <?= $r['exam'] ?>, <?= $r['city'] ?></p>
            </div>
            <?php endforeach; ?>
        </div>
        <p class="text-[10px] text-gray-400 mt-1 text-center">← Swipe karo →</p>
    </div>
</section>

<!-- ========== PAIN AGITATE ========== -->
<section class="py-10 sm:py-12 md:py-16 bg-parchment">
